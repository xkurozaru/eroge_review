...
レビューの採点を監視するために、月次でスコア分布（平均・標準偏差・件数）のスナップショットを保存できます。

- Endpoint（Console Server）: `GET /internal/cron/review-score-stats/monthly`
- Query: `?date=YYYY-MM-DD`（省略時は JST の「当月の月初」を集計して保存）
- 実行頻度: 毎日。既存レコードがあれば UPDATE、なければ INSERT します。

レスポンス:

- `{"stats_month":"YYYY-MM-DD"}`
